...
const ProPage = () => {
	...
	const handlePlanSelection = async (planId: 'month' | 'year') => {
		if (!user) {
			toast.warning('Please login to select a plan.', {
				id: 'login-warning',
				position: 'top-center',
				duration: 3000
			});
		...
		try {
			...
			if (
				...
				toast.error(
					'You have tried too many times. Please try again'
				);
			} else {
				toast.error(
	...
	return (
		<div className="container mx-auto px-4 py-16 max-w-6xl h-screen">
			...
			<p className="text-xl text-center mb-12 text-gray-600 dark:text-gray-200">
				Unlock premium features and accelerate your
				learning
			</p>
			...
			<div className="grid md:grid-cols-2 gap-8 items-stretch">
				{PRO_PLANS.map(plan => (
					<Card
						key={plan.id}
						className={`
              flex flex-col transition-all duration-300 dark:bg-slate-100 ${
			plan.highlighted
